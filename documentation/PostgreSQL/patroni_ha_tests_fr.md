...
- [Analyse des logs /tmp/patroni.log](#analyse-des-logs-/tmp/patroni.log)
...
## Analyse des logs /tmp/patroni.log
...
```text
2020-07-09 14:02:35,835 WARNING: Traceback (most recent call last):
...
|:---|:---|:---|:---|
| 1 | Tuer le processus PostgreSQL | `ps -edf\|grep '[/]usr/pgsql-11//bin/postgres' \| awk '{ print $2}' \| xargs kill -9` | Patroni relance le serveur PostgreSQL automatiquement. Le service reste opérationnel. |
| 2 | Arrêt du serveur PostgreSQL | `pgstop tarif` | Patroni relance le serveur PostgreSQL automatiquement. |
...
| 4 | Arrêt du service Patroni | `# systemctl stop patroni@tarif.service` | Le serveur PostgreSQL est arrêté en même temps. Pas de bascule (failover) car le primaire est sain. |
...
| 2 | Arrêt du serveur PostgreSQL | `pgstop tarif` | Patroni relance PostgreSQL automatiquement. Le nœud reste Primaire. |
| 3 | Reboot du serveur Linux | `# reboot` | Un nœud standby est réélu Primaire. Le service est maintenu par le nouveau leader. |
| 4 | Arrêt du service Patroni | `# systemctl stop patroni@tarif` | PostgreSQL est arrêté. Un autre nœud est automatiquement élu Primaire. Continuité de service assurée. |
...
| ID | Scénario de test | Commandes | Observations |
|:---|:---|:---|:---|
| 1 | Isoler port PG (6003) - DROP | `iptables -A INPUT -p tcp --dport 6003 -j DROP` | Le serveur disparaît de `patronictl list`. Blacklisté par le Load Balancer. |
...
| 2 | Isoler port Patroni (8003) - DROP | `iptables -A INPUT -p tcp --dport 8003 -j DROP` | Blacklisté par HAProxy. `patronictl list` ne détecte pas immédiatement la perte. |
...
| 3 | Isoler port PG (6003) - REJECT | `iptables -A INPUT -p tcp --dport 6003 -j REJECT` | Comportement similaire au DROP. Détection après quelques minutes. |
| 4 | Isoler port Patroni (8003) - REJECT | `iptables -A INPUT -p tcp --dport 8003 -j REJECT` | Comportement similaire au DROP. Bascule vers nœud sain possible. |

## Tests isolation de nœud Primaire (Blocage TCP)

| ID | Scénario de test | Commandes | Observations |
|:---|:---|:---|:---|
| 1 | Isoler port PG (6003) - DROP | `iptables -A INPUT -p tcp --dport 6003 -j DROP` | Perte du leader. Une réélection est déclenchée. Nouveau leader promu. |
| 5 | Isoler port PG (6003) sur Primaire | `iptables -A INPUT -p tcp --dport 6003 -j DROP` | Les processus ne s'arrêtent pas, mais l'API ne répond plus. Nouveau leader élu après expiration du TTL. |
| 6 | Isoler port Patroni (8003) sur Primaire | `iptables -A INPUT -p tcp --dport 8003 -j DROP` | **Attention** : Pas de bascule automatique immédiate car Patroni ne se voit pas défaillant ! |
| 7 | Isoler port PG (6003) - REJECT | `iptables -A INPUT -p tcp --dport 6003 -j REJECT` | Nouveau leader réélu suite à l'impossibilité d'accéder à PostgreSQL. |
| 8 | Isoler port Patroni (8003) - REJECT | `iptables -A INPUT -p tcp --dport 8003 -j REJECT` | Pas de bascule automatique du leader. Isolation au niveau Proxy uniquement. |

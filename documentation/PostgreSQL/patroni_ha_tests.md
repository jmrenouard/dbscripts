...
- [Log Analysis /tmp/patroni.log](#log-analysis-/tmp/patroni.log)
...
|:---|:---|:---|:---|
| 1 | Kill PostgreSQL process | `ps -edf\|grep '[/]usr/pgsql-11//bin/postgres' \| awk '{ print $2}' \| xargs kill -9` | Patroni automatically restarts the PostgreSQL server. Service remains operational. |
| 2 | Stop PostgreSQL server | `pgstop tarif` | Patroni automatically restarts the PostgreSQL server. |
| 3 | Linux Server Reboot | `# reboot` | If Patroni service is `enabled`, it starts PostgreSQL on boot. No global service loss. |
| 4 | Stop Patroni service | `# systemctl stop patroni@tarif.service` | The PostgreSQL server is stopped at the same time. No failover as primary is healthy. |
...
| 2 | Stop PostgreSQL server | `pgstop tarif` | Patroni automatically restarts PostgreSQL. Node remains Primary. |
| 3 | Linux Server Reboot | `# reboot` | A standby node is re-elected as Primary. Service maintained by the new leader. |
| 4 | Stop Patroni service | `# systemctl stop patroni@tarif` | PostgreSQL is stopped. Another node is automatically elected as Primary. Service continuity ensured. |
...
| ID | Test Scenario | Commands | Observations |
|:---|:---|:---|:---|
| 1 | Isolate PG port (6003) - DROP | `iptables -A INPUT -p tcp --dport 6003 -j DROP` | Server disappears from `patronictl list`. Blacklisted by Load Balancer. |
...
| 2 | Isolate Patroni port (8003) - DROP | `iptables -A INPUT -p tcp --dport 8003 -j DROP` | Blacklisted by HAProxy. `patronictl list` does not immediately detect loss. |
...
| 3 | Isolate PG port (6003) - REJECT | `iptables -A INPUT -p tcp --dport 6003 -j REJECT` | Similar behavior to DROP. Detection after a few minutes. |
| 4 | Isolate Patroni port (8003) - REJECT | `iptables -A INPUT -p tcp --dport 8003 -j REJECT` | Similar behavior to DROP. Switchover to healthy node possible. |

## Primary Node Isolation Tests (TCP Blocking)

| ID | Test Scenario | Commands | Observations |
|:---|:---|:---|:---|
| 1 | Isolate PG port (6003) - DROP | `iptables -A INPUT -p tcp --dport 6003 -j DROP` | Leader lost. A re-election is triggered. New leader promoted. |
| 5 | Isolate PG port (6003) on Primary | `iptables -A INPUT -p tcp --dport 6003 -j DROP` | Processes don't stop, but API stops responding. New leader elected after TTL. |
| 6 | Isolate Patroni port (8003) on Primary | `iptables -A INPUT -p tcp --dport 8003 -j DROP` | **Warning**: No immediate automatic failover as Patroni doesn't see itself as failing! |
| 7 | Isolate PG port (6003) - REJECT | `iptables -A INPUT -p tcp --dport 6003 -j REJECT` | New leader re-elected due to PostgreSQL inaccessibility. |
| 8 | Isolate Patroni port (8003) - REJECT | `iptables -A INPUT -p tcp --dport 8003 -j REJECT` | No automatic leader failover. Isolation at Proxy level only. |
